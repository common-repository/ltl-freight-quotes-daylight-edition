...
namespace EnDaylightPackage;

use EnDaylightDistance\EnDaylightDistance;
use EnDaylightProductDetail\EnDaylightProductDetail;
use EnDaylightReceiverAddress\EnDaylightReceiverAddress;
use EnDaylightWarehouse\EnDaylightWarehouse;

/**
 * Get items detail from added product in cart|checkout page.
 * Class EnDaylightPackage
 * @package EnDaylightPackage
 */
if (!class_exists('EnDaylightPackage')) {
    ...
    class EnDaylightPackage
    {
        ...
        {
            $product_detail_obj = new EnDaylightProductDetail();
            ...
            self::$receiver_address = EnDaylightReceiverAddress::get_address();
            ...
            foreach ($package['contents'] as $key => $product) {
                if (isset($product['data'])) {
                    ...
                    // Flat rate pricing
                    $en_flat_rate_price = EnDaylightPackage::en_get_flat_rate_price($product, $product_data);
                    ...
                    $origin_zip_code = '';
                    $shipment_type = EN_DAYLIGHT_DECLARED_FALSE;
                    // Micro Warehouse
                    ...
                    foreach ($en_product_fields as $en_field_key => $en_custom_product) {
                        ...
                        if (is_callable(array(self::class, $en_function_trigger)) && strlen($is_line_item) > EN_DAYLIGHT_DECLARED_ZERO) {
                            ...
                            $suscription_and_features = apply_filters(
                                "daylight_plans_suscription_and_features", $is_plans
                            );
                            ...
                            (!is_array($en_location_value) &&
                                strlen($en_location_value) > EN_DAYLIGHT_DECLARED_ZERO) ? $product_item[$is_line_item] = $en_location_value : "";
                            ...
                            if (isset($en_location_value['senderZip']) &&
                                ...
                                switch (EN_DAYLIGHT_DECLARED_TRUE) {
                                    case $shipping_class == 'ltl_freight':
                                        $shipment_type = EN_DAYLIGHT_DECLARED_TRUE;
                                        self::en_set_ltl_shipment($origin_zip_code);
                                        self::$en_request['LTL_FREIGHT'] = EN_DAYLIGHT_DECLARED_ONE;
                                        break;
                                    case $shipment_weight > EN_DAYLIGHT_SHIPMENT_WEIGHT_EXCEEDS_PRICE && EN_DAYLIGHT_SHIPMENT_WEIGHT_EXCEEDS == 'yes':
                                        self::en_set_ltl_shipment($origin_zip_code);
                                        $shipment_type = EN_DAYLIGHT_DECLARED_TRUE;
                                        $product_weight < EN_DAYLIGHT_SHIPMENT_WEIGHT_EXCEEDS_PRICE ?
                                            self::$en_request['shipment_type']['LTL_SMALL'][$origin_zip_code]['SMALL'] = EN_DAYLIGHT_DECLARED_TRUE : "";
                                        break;
                                    case $product_weight < EN_DAYLIGHT_SHIPMENT_WEIGHT_EXCEEDS_PRICE:
                                        $shipment_type = EN_DAYLIGHT_DECLARED_TRUE;
        ...
        {
            $reasons = !self::$shipment_type ? array_merge($reasons, [EN_DAYLIGHT_712]) : $reasons;
            return (!self::$origin_zip_code > 0) ? array_merge($reasons, [EN_DAYLIGHT_713]) : $reasons;
        }
        ...
        static public function en_filter_shipment()
        {
            ...
            self::$en_request = apply_filters("en_woo_addons_carrier_service_quotes_request", self::$en_request, EN_DAYLIGHT_SHIPPING_NAME);
            // standard packaging
        ...
        {
            self::$en_request['shipment_type']['LTL_FREIGHT'][$origin_zip_code]['LTL'] = self::$en_request['shipment_type']['LTL_SMALL'][$origin_zip_code]['LTL'] = EN_DAYLIGHT_DECLARED_TRUE;
        }
        ...
        {
            self::$en_request['shipment_type']['LTL_FREIGHT'][$origin_zip_code]['SMALL'] = self::$en_request['shipment_type']['LTL_SMALL'][$origin_zip_code]['SMALL'] = EN_DAYLIGHT_DECLARED_TRUE;
        }
        ...
        static public function en_is_instore_pickup_enabled($location, $sender_location_address)
        {
            ...
            $suscription_and_features = apply_filters(
                "daylight_plans_suscription_and_features", 'instore_pickup_local_delivery'
            );
         ...
         */
        static public function en_daylight_get_location($location_id = '')
        {
            ...
            $en_location = EnDaylightWarehouse::get_data($en_where_clause);
            // Micro Warehouse
            if (stripos(implode($all_plugins), 'micro-warehouse-shipping.php')  || is_plugin_active_for_network('micro-warehouse-shipping-for-woocommerce/micro-warehouse-shipping.php')) {
            ...
            if (!empty($en_location) && is_array($en_location)) {
                if (count($en_location) == 1) {
                    ...
                    $get_address = json_decode(
                        EnDaylightDistance::get_address($en_location, $en_access_level, $receiver_address), true);
                    ...
                    switch ($enable_dropship) {
                        case 'yes':
                            ...
                            if (stripos(implode($all_plugins), 'micro-warehouse-shipping.php')  || is_plugin_active_for_network('micro-warehouse-shipping-for-woocommerce/micro-warehouse-shipping.php')) {
                                $locations_dropship = maybe_unserialize(get_post_meta(self::$post_id, '_dropship_location', true));
                                if(!empty($locations_dropship)) {
                                    self::$get_minimum_warehouse = self::en_daylight_get_location();
                                    return self::$get_minimum_warehouse;
                                }
                            }
                            return self::en_daylight_get_location(get_post_meta(self::$post_id, self::$product_key_name, true));
                        default:
                            self::$get_minimum_warehouse = self::en_daylight_get_location();
                            return self::$get_minimum_warehouse;
                    }
